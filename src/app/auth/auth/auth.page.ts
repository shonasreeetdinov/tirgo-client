...
export class AuthPage implements OnInit {
    ...
    openCountryModal() {
        this.modalCtrl.create({ component: this.countryModal }).then((modal) => {
          modal.present();
        });
      }
      formatPhone() {
        // Format the phone number according to the selected country's mask
        const cleanPhone = this.phone.replace(/\D/g, '');
        let formattedPhone = '';
        let maskIndex = 0;
        let phoneIndex = 0;
    
        while (maskIndex < this.mask.length && phoneIndex < cleanPhone.length) {
          if (this.mask[maskIndex] === '0') {
            formattedPhone += cleanPhone[phoneIndex];
            phoneIndex++;
          } else {
            formattedPhone += this.mask[maskIndex];
          }
          maskIndex++;
        }
    ...
    async verifyCode() {
        ...
        await this.authService.verifyCode(this.prefix + '' + this.phone, this.code).toPromise()
            .then(async (res) => {
                if (res.status) {
                    console.log(res)
                    await this.authService.setJwt(res.token, res.refreshToken);
                    // this.loading = false;
